    ...
    <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/70 backdrop-blur-sm p-4">
      <div className="bg-neutral-900 border border-neutral-700 rounded-2xl w-full max-w-md overflow-hidden animate-slide-up">
        ...
        <div className="p-6 pb-2 text-center">
          <div className="inline-flex items-center justify-center w-12 h-12 bg-rose-600/20 rounded-full mb-3">
          ...
          {step === 0 && (
            <>
              ...
              <button
                onClick={() => setStep(1)}
                disabled={!gender}
                className="w-full mt-4 flex items-center justify-center gap-2 px-4 py-2.5 bg-rose-600 hover:bg-rose-700 disabled:opacity-40 disabled:cursor-not-allowed text-white text-sm font-medium rounded-xl transition-colors"
              >
                {t('onboarding.next')}
          ...
          {step === 1 && (
            <>
              ...
              <button
                ...
                className="w-full mt-4 flex items-center justify-center gap-2 px-4 py-2.5 bg-rose-600 hover:bg-rose-700 text-white text-sm font-medium rounded-xl transition-colors"
              >
                {t('onboarding.finish')}
                <Sparkles size={16} />
              </button>
          ...
          {[0, 1].map((s) => (
            <div
              ...
              className={`w-1.5 h-1.5 rounded-full transition-colors ${
                s === step ? 'bg-rose-500' : 'bg-neutral-700'
              }`}
            />
          ))}
        </div>
