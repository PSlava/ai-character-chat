...
- **`components/chat/GenerationSettingsModal.tsx`** — Modal with model card grid (auto, openrouter, groq, cerebras, together, direct, paid groups) + 5 sliders (temperature, top_p, top_k, frequency_penalty, max_tokens).
...
- **Literary prose format**: Prompts instruct the model to write as literary prose — narration as plain text, dialogue via em-dash (ru) / quotes (en), `*asterisks*` only for inner thoughts. Show-don't-tell, physical sensations, paragraph breaks for rhythm. Explicit ban on template pattern `*does X* text *does Y*`.
