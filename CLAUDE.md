...
- **`characters/`** — CRUD + AI generation from text + SillyTavern export/import. Tags as comma-separated string. `structured_tags.py` — registry of 33 predefined tags in 5 categories with bilingual labels and prompt snippets. `language_preferences.py` — per-language affinity weights for settings/tags/ratings, used for initial base counts and daily growth. `serializers.py` for ORM→dict with `getattr` fallbacks for new columns, inflated counters (`real + base[lang]`), admin sees `real_chat_count`/`real_like_count`. Admin bypass for edit/delete via `is_admin` param.
...
- **Autonomous scheduler**: `asyncio.create_task(run_scheduler())` in lifespan. Hourly check, 24h intervals. State in `prompt_templates` with `scheduler.*` keys. Tasks: daily character generation (LLM + DALL-E avatar + auto-translate), counter growth, cleanup. Email admin on character generation failure. Config: `AUTO_CHARACTER_ENABLED` env var.
