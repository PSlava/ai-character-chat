...
- **`admin/router.py`** — Admin-only CRUD for prompt template overrides. `require_admin` dependency checks JWT role. GET/PUT/DELETE `/api/admin/prompts`. Admin settings: `GET/PUT /api/admin/settings` (key-value in `prompt_templates` with `setting.` prefix). Settings: `notify_registration`, `paid_mode`, `daily_message_limit`, `max_personas`.
...
- **Admin settings**: Key-value pairs stored in `prompt_templates` with `setting.` prefix. `GET/PUT /api/admin/settings`. Current settings: `notify_registration` (email on new user), `paid_mode` (paid LLM providers). Toggle buttons on AdminUsersPage (Bell for notifications, DollarSign for paid mode).
...
Chats: `POST /api/chats` (get-or-create, `force_new` for multiple chats), `GET /api/chats`, `GET/DELETE /api/chats/{id}`, `GET /api/chats/{id}/messages?before=ID&limit=20` (pagination), `POST /api/chats/{id}/message` (SSE), `POST /api/chats/{id}/generate-persona-reply` (ghostwrite as persona), `DELETE /api/chats/{id}/messages` (clear), `DELETE /api/chats/{id}/messages/{msg_id}`
