...
- **`autonomous/`** — Autonomous server tasks (no cron/Celery):
  ...
  - `character_generator.py` — Daily character generation: weighted categories (fantasy 30%, modern 30%, anime 15%, sci-fi 5%, supernatural 5%, moderate 5%), LLM invents unique concept + avatar prompt (Groq→Cerebras→OpenRouter, temp=0.95), DALL-E 3 avatar ($0.04/day), auto-translation to EN/ES, saves under @sweetsin user. ~90% NSFW.
  ...
  - `highlight_generator.py` — Daily: generates 2 editorial highlight phrases per language (ru/en/es) for up to 10 characters without highlights. Groq→Cerebras→OpenRouter fallback. NOT fake reviews — editorial descriptive phrases.
  - `relationship_builder.py` — Weekly: finds character pairs with 2+ shared tags, LLM determines relation type (rival/ex/friend/sibling/enemy/lover/ally), creates bidirectional relations with trilingual labels. Max 3 relations per character, 20 pairs per run.
