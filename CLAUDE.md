...
- **Literary prose format**: Prompts instruct the model to write as literary prose — **third-person narration** per language (she/he, elle/il, ella/él, sie/er, ela/ele, lei/lui). Dialogue format varies by language: em-dash with space `— Text` (ru/fr/pt/it), em-dash without space `—Text` (es), quotes `"..."` (en), typographic quotes `„..."` (de). `*asterisks*` only for inner thoughts. Show-don't-tell, physical sensations, paragraph breaks for rhythm. Explicit ban on template pattern `*does X* text *does Y*`.
...
- **SEO prerender**: Nginx user-agent match (Googlebot, Bingbot etc.) → backend HTML for /:lang/c/:slug, /:lang/tags/:slug, /:lang/faq, /:lang (home). Languages: `(en|es|ru|fr|de|pt|it)` in nginx regex.
