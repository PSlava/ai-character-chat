...
- **`characters/`** — CRUD + AI generation from text + SillyTavern export/import + voting + forking. Tags as comma-separated string. `structured_tags.py` — registry of 33 predefined tags in 5 categories with multilingual labels and prompt snippets. `language_preferences.py` — per-language affinity weights for settings/tags/ratings, used for initial base counts and daily growth. `slugify.py` — `validate_slug()` (normalize, validate 3-50 chars). `serializers.py` for ORM→dict with `getattr` fallbacks for new columns, inflated counters (`real + base[lang]`), `vote_score`, `fork_count`, `forked_from_id`, `highlights`. Admin sees `real_chat_count`/`real_like_count`. Admin bypass for edit/delete via `is_admin` param. `router.py` includes `GET /check-slug` (availability), `POST /{id}/vote` (upvote/downvote/remove), `POST /{id}/fork` (clone public char as private draft), `GET /{id}/relations` (character connections).
...
- **`api/characters.ts`** — `wakeUpServer()` pings `/health` every 3s for up to 3 min. `getOpenRouterModels()` fetches model registry. `voteCharacter()`, `getUserVotes()`, `forkCharacter()`, `getCharacterRelations()`, `checkSlug()` (debounced availability check).
...
- **`components/characters/CharacterForm.tsx`** — Full character form with appearance, structured tag pills (fetched from API, grouped by category), and `{{char}}`/`{{user}}` hint in example dialogues placeholder. User-defined slug with debounced availability check. Admin-only fields: preferred_model, response_length, max_tokens slider, system_prompt_suffix.
...
- **RSS feed**: `/feed.xml` — RSS 2.0, 30 latest characters with avatar enclosures and EN translations for names/descriptions. Nginx proxies to `/api/seo/feed.xml`. `<link rel="alternate">` in index.html.
...
- **Character slug**: User-defined, unique per creator (`UniqueConstraint('creator_id', 'slug')`). Validated via `validate_slug()` (3-50 chars, lowercase alphanumeric + hyphens). Frontend: debounced 500ms availability check. Backend: `GET /characters/check-slug`, auto-generated fallback if not provided.
...
- **Admin-only UI**: CharacterForm hides preferred_model, response_length, max_tokens, system_prompt_suffix from non-admin users. Persona generation (Sparkles button) is admin-only in ChatPage.
...
Analytics: `POST /api/analytics/pageview` (public, rate-limited), `GET /api/admin/analytics/overview?days=7` (admin — traffic, countries, devices, models, traffic_sources, os, bot_views, anon_stats)
