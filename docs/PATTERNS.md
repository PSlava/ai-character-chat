...
- **Model 404 auto-fix**: `model_resolver.py` catches model-not-found errors at runtime. `FIXABLE_PROVIDERS = {"grok", "mistral", "deepseek", "openai"}` — providers with `client.models.list()`. On 404: fetches model list → token-overlap matching → saves override to DB (`prompt_templates` with `model_override.*` keys) → retries request → emails admin. In-memory `_overrides` dict loaded from DB on startup. 10-min cooldown per provider. Explicit-provider path does full retry with buffering/refusal check; auto-mode fires fire-and-forget resolve. Daily health check also auto-fixes on 404.
...
- **Response length**: Character setting (`short`/`medium`/`long`/`very_long`) controls system prompt instructions about response format and length. Separate from `max_tokens` which is a hard token limit. **Provider hint length enforcement**: `_LENGTH_REMINDER` dict in router.py appends compact length reminders (per level × 7 languages) to every provider hint — ensures length instruction is in strongest position (last system message, closest to generation).
...
- **SillyTavern export/import**: Character card v2 format (`spec: "chara_card_v2"`). Export: `GET /api/characters/{id}/export` with Content-Disposition (**admin-only**, 403 for non-admin). Import: `POST /api/characters/import` from JSON card (v1/v2). Fork: `POST /api/characters/{id}/fork` (**admin-only**). SweetSin-specific fields in `extensions.sweetsin`. Import tab on CreateCharacterPage (file upload or JSON paste). Frontend: export link and fork button hidden behind `isAdmin` check on CharacterPage.
...
- **Online status (fake)**: `isCharacterOnline(id)` in `lib/utils.ts` — deterministic hash of `id + currentHour`, ~33% show green dot. Used on CharacterCard and CharacterPage. Online indicator moves to top-right when companion badge is present.
- **Companion NPC**: 5 fields (`companion_name`, `companion_role`, `companion_personality`, `companion_appearance`, `companion_avatar_url`). All in `_NULLABLE_CLEARABLE_FIELDS`. Avatar uses same pipeline as main avatar (512px + 160px thumb WebP). CharacterCard: 32px badge at bottom-right of main avatar. CharacterPage: 36px clickable badge → `ImageLightbox`. CharacterForm: `AvatarUpload` shown when companion enabled + name entered. Fork copies `companion_avatar_url`.
