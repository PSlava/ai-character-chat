  ...
  return (
    <>
      ...
      <section className="relative overflow-hidden">
        <div className="absolute inset-0 bg-gradient-to-b from-blue-950/30 via-neutral-900 to-neutral-900" />
        <div className="relative max-w-5xl mx-auto px-4 pt-12 pb-16 md:pt-20 md:pb-24 text-center">
          <Logo className="w-16 h-16 md:w-20 md:h-20 text-blue-500 mx-auto mb-6" />

          <h1 className="text-4xl md:text-6xl font-bold mb-4">
            <span className="text-white">Sweet</span>
            <span className="text-blue-500">Sin</span>
          </h1>

          <p className="text-xl md:text-2xl text-blue-300/80 font-medium mb-3">
            {t('hero.slogan')}
          </p>
                ...
                {avatarChars.map((char) => (
                  <Link to={localePath(char.slug ? `/c/${char.slug}` : `/character/${char.id}`)} key={char.id} title={char.name}>
                    <img
                      src={char.avatar_url!}
                      alt={char.name}
                      className="w-12 h-12 md:w-14 md:h-14 rounded-full object-cover border-2 border-neutral-700 hover:border-blue-500 transition-colors"
                    />
                  </Link>
                ))}
              </div>
      ...
      <section className="max-w-5xl mx-auto px-4 pb-8">
        <div className="grid grid-cols-2 md:grid-cols-3 gap-3 md:gap-4">
          {features.map((f) => (
            ...
            >
              <div className="w-10 h-10 rounded-lg bg-blue-600/10 flex items-center justify-center mx-auto mb-3">
