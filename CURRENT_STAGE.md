...
| WEB_WORKERS | — | 2 | Кол-во gunicorn workers (1 для серверов с малым RAM) |
...
| GOOGLE_CLIENT_ID | — | Вручную | Google OAuth Client ID |
| GOOGLE_CLIENT_SECRET | — | Вручную | Google OAuth Client Secret |
...
- **Google OAuth** — авторизация через Google аккаунт (authlib):
  - `GET /api/auth/google` → Google consent → callback → JWT → redirect на `/auth/oauth-callback`
  - Автоматическая привязка существующего аккаунта по email
  - Создание нового пользователя с `oauth_provider`/`oauth_id`
  - `password_hash` nullable для OAuth-only пользователей
  - Фронтенд: кнопка «Войти через Google» на AuthPage, OAuthCallbackPage для обработки токена
...
### Чат
- **Множественные чаты с одним персонажем** — кнопка «Новый чат» на страницах персонажа и в шапке чата; `force_new: true` при создании чата пропускает поиск существующего
- **Группировка чатов** — sidebar группирует чаты по персонажу, нумерация `#N` для множественных чатов
...
- **System prompt (литературный формат):**
  ...
  - **Fiction author frame** — "Ты — опытный автор современной прозы" расширен на SFW/moderate (было только NSFW)
  - **NSFW scene pacing** — "Интимные сцены как музыка: после интенсивных моментов — пауза" (7 языков)
...
- **Перегенерация ответа** — кнопка на hover + постоянная кнопка под последним ответом. `is_regenerate: true` пропускает 5-сек cooldown между сообщениями. HTTP-ошибки (429/500) показывают реальный текст вместо generic "Connection error"
...
### Групповые чаты
- **1 пользователь + 2-3 AI-персонажа** — персонажи отвечают последовательно через SSE
- **Модели данных**: `GroupChat`, `GroupChatMember` (position для порядка), `GroupMessage` (character_id для атрибуции)
- **API**: `POST/GET/DELETE /api/group-chats`, `POST /api/group-chats/{id}/message`
- **SSE-события**: `user_saved` → `character_start`/`token`/`character_done` (для каждого персонажа) → `all_done`, `character_error`
- **Разделение ролей**: сообщения текущего персонажа → `role: assistant`, сообщения других → `role: user` с префиксом `[Имя]:` (предотвращает продолжение чужого текста)
- **Групповой контекст**: локализованная инструкция (7 языков): "Отвечай ТОЛЬКО как {name}", "СТРОГО 1-2 абзаца"
- **Post-history reminder** — `_POST_HISTORY` из `service.py` инжектируется последним системным сообщением
- **Ограничения**: `MAX_MEMBERS=3`, `max_tokens=600`, `MAX_CONTEXT_MESSAGES=30`
- **Язык**: "Пиши ТОЛЬКО на русском" (для ru) — против вставки английских слов (артефакт Llama)
- **Фронтенд**: `CreateGroupChatModal` (поиск + чекбоксы), `GroupChatPage` (streaming, typing indicators per character)
- **Sidebar**: секция "Групповые чаты" с оверлэппинг аватарами
...
- **Система жалоб (Report)** — пользователи могут жаловаться на персонажей:
  - `POST /api/characters/{id}/report` — создание жалобы (причина + описание), rate limit 5/час
  - 5 причин: inappropriate, spam, impersonation, underage, other
  - Статусы: pending → reviewed / dismissed
  - `GET /api/admin/reports?status=` — список жалоб с фильтрацией для админа
  - `PUT /api/admin/reports/{id}` — изменение статуса
  - Фронтенд: кнопка Flag (🚩) на странице персонажа, ReportModal с радио-кнопками причин
...
| Kimi K2-0905 | 8/10 | 1T MoE (32B active), 256K контекст |
| Llama 4 Scout/Maverick | 7/10 | Новые |
| GPT-OSS 120B | 6/10 | Строгая модерация, NSFW заблокирован |
| Qwen3 32B | 5/10 | CJK в русском тексте, ниже порога auto-fallback |
...
- **NSFW anti-repetition** — промпты инструктируют выбирать 1-2 чувства на ответ и чередовать их, запрещают повторять описания ощущений из предыдущих ответов
...
- [ ] **Токен-экономика** — токены за premium-действия (генерация картинок, голос, длинный контекст)
- [ ] **Подписки (3 тира)** — Free (лимит + базовые модели), Plus $9.99 (безлимит, без рекламы), Pro $19.99 (голос, картинки включены)
- [ ] **AI-картинки в чате** — "пришли селфи в красном платье" → DALL-E генерирует. 2-4 токена за картинку (SpicyChat, Candy.AI)
- [ ] **Голосовые сообщения/звонки** — TTS/STT через ElevenLabs, оплата за минуту (Candy.AI, Character.AI)
...
- [x] ~~Кеш приветственных сообщений (greeting cache) — при создании чата используется кешированный перевод greeting_message из JSONB translations (язык передаётся с фронтенда)~~
...
│   │   ├── characters/              # CRUD + генерация + теги + перевод + экспорт/импорт
│   │   │   ├── router.py            # API endpoints (admin bypass) + GET /structured-tags + export/import
...
│   │   │   ├── chat.ts              # chats, messages, delete, clear, forceNew
│   │   │   ├── reports.ts           # createReport, getReports, updateReport
│   │   │   ├── export.ts            # getExportUrl, importCharacter (SillyTavern)
...
│   │   ├── pages/                   # 20 страниц
│   │   │   ├── AuthPage.tsx         # Вход / регистрация / забыли пароль + Google OAuth
│   │   │   ├── OAuthCallbackPage.tsx # Обработка OAuth redirect (token из URL → store)
│   │   │   ├── ResetPasswordPage.tsx # Установка нового пароля (по ссылке из email)
│   │   │   ├── ChatPage.tsx         # Чат + настройки + модель + isAdmin + новый чат
...
│   │   │   │   ├── MessageBubble.tsx     # Сообщение + delete + regenerate + copy + markdown + model_used (admin)
...
│   │   │   │   └── ReportModal.tsx       # Модалка жалобы (5 причин + описание)
│   │   │   ├── landing/
