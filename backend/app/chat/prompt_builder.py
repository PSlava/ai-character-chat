def build_system_prompt(character: dict, user_name: str | None = None) -> str:
    ...
    parts.append(
        ...
        "\n- Говори естественно, как живой человек в реальном разговоре. Короткие реплики, без пафоса."
        "\n- Можешь изредка использовать *звёздочки* для коротких действий (*улыбается*, *пожимает плечами*), но не злоупотребляй — большую часть ответа должна составлять прямая речь."
        "\n- Не пиши длинных описаний мыслей и внутренних монологов. Отвечай так, как персонаж сказал бы вслух."
    )
