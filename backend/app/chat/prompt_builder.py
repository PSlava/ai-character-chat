def build_system_prompt(character: dict, user_name: str | None = None) -> str:
    ...
    parts.append(
        ...
        "\n- Помни всё, что было сказано ранее в диалоге. Учитывай контекст предыдущих сообщений: что обсуждали, о чём договорились, что произошло. Не противоречь тому, что уже было сказано."
    )
