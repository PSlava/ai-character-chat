...
Верни строго JSON-объект со следующими полями:
{
  ...
  "speech_pattern": "Речевой стиль: акцент, уровень формальности, любимые слова/фразы, речевые причуды, особенности произношения. Оставь пустым если нет ярких особенностей.",
...
async def fork_character(
    ...
    forked = Character(
        ...
        speech_pattern=getattr(original, 'speech_pattern', None),
        backstory=getattr(original, 'backstory', None),
        hidden_layers=getattr(original, 'hidden_layers', None),
        inner_conflict=getattr(original, 'inner_conflict', None),
        companion_name=getattr(original, 'companion_name', None),
...
async def update_character(
    ...
    # Identify changed translatable fields BEFORE service mutates data dict (pops tags)
    _translatable = ("name", "tagline", "personality", "scenario", "appearance", "greeting_message")
    changed_fields = [k for k in _translatable if data.get(k) is not None]
    ...
    # Trigger background translation for only the changed fields
    if changed_fields:
        import asyncio
        from app.characters.translation import translate_character_all_languages
        asyncio.create_task(translate_character_all_languages(character_id, changed_fields=changed_fields))
