...
{{
  ...
  "backstory": "3-5 предложений. (1) ОДНО конкретное событие из прошлого (Ghost), которое сформировало персонажа. НЕ эпическая трагедия - тихое предательство, потеря, ошибка. (2) Эмоциональная рана (Wound) - как это событие влияет на поведение СЕЙЧАС. (3) Ложное убеждение (Lie), принятое как защита. (4) Секрет - что персонаж скрывает от всех.",
  "hidden_layers": "4 уровня доверия. Формат СТРОГО: Level 1: [поведение с незнакомцем - маска, защита] Level 2: [~10 сообщений - начинает делиться мнениями, проскальзывают намёки на прошлое] Level 3: [~20 сообщений - раскрывает рану и уязвимость] Level 4: [30+ сообщений - главный секрет, ломка Лжи]",
  "inner_conflict": "1-2 предложения. ХОЧЕТ: [внешняя цель, видимая] vs НУЖДАЕТСЯ: [внутренний рост, скрытый]. Они должны КОНФЛИКТОВАТЬ.",
...
СТИЛЬ:
- Пиши живым языком. Вместо абстрактных описаний - конкретные детали. Вместо 'загадочный' - покажи загадочность через действия
- Каждое предложение - новая информация. Никаких штампов
- НИКОГДА не используй длинное тире '-', только обычный дефис '-'
- personality: ПОВЕДЕНИЕ, а не прилагательные. 'отвечает вопросом на вопрос' лучше чем 'умный'. 'не смотрит в глаза' лучше чем 'застенчивый'
...
- greeting_message: персонаж ДЕЛАЕТ что-то, а не стоит и ждёт. Заверши вызовом, не вопросом 'чем я могу помочь?'
- example_dialogues: 2-3 примера РАЗНЫХ настроений - не только одну эмоцию
- appearance: начни с детали, которая раскрывает характер (шрам, мозоли, одежда), а не с цвета глаз
- backstory создаёт ДРАМУ - конкретное событие, рана, ложное убеждение, секрет. НЕ эпика, а тихая личная боль
- hidden_layers дают LONGEVITY - 4 уровня поведения от маски до полной открытости. Формат: Level 1: ... Level 2: ... Level 3: ... Level 4: ...
- inner_conflict дает НАПРЯЖЕНИЕ - внешняя цель vs внутренняя потребность, они ПРОТИВОРЕЧАТ друг другу
- Для NSFW: НЕЛЬЗЯ описывать сексуальное поведение в personality - оно раскрывается только через сценарий и диалог"""
