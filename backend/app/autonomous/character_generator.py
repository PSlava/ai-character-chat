...
{{
  ...
  "speech_pattern": "речевой стиль: акцент, уровень формальности, любимые слова, речевые причуды. Пустая строка если нет ярких особенностей",
  "scenario": "сцена первой встречи, 2-3 предложения",
  "greeting_message": "первое сообщение в литературном формате: 3-е лицо (он/она), действия обычным текстом, диалоги через обычное тире '-', мысли через *звёздочки*, 3-4 абзаца, физические действия обязательны. НИКОГДА не используй длинное тире '—', только обычный дефис '-'",
...
- personality ОБЯЗАТЕЛЬНО содержит 1-2 внутренних противоречия (жёсткий снаружи — уязвимый внутри, и т.п.)
- personality должен включать конкретные привычки и странности (крутит кольцо когда нервничает, не выносит тишину)
- speech_pattern: если у персонажа есть яркий стиль речи — опиши (акцент, любимые словечки, манера). Если нет — пустая строка
- greeting_message: показывай через физические детали (запах, текстура, звуки), а не через абстрактные описания
- example_dialogues: покажи 2-3 разных грани персонажа (не только одну эмоцию)
