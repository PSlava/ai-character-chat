...
_CHAR_GEN_PROVIDERS = ("openai", "claude", "deepseek", "together", "groq")
...
ВАЖНО — ИМЯ ПЕРСОНАЖА (всегда на кириллице):
- Фэнтези/аниме → уникальное фэнтезийное имя на кириллице (Элоуэн, Зефир, Лира, Серафиэль, Вивьен, Азраэль, Раэль, Иштар, Ноктис)
- Современность/эротика → международное имя на кириллице (Алекс, Луна, Софи, Мила, Лео, Этан, Майя, Ренцо, Марко)
- НИКОГДА не используй типичные русские имена (Катя, Настя, Юля, Вера, Дима, Саша, Лена, Ира, Оксана)
- НИКОГДА не пиши имя латиницей — только кириллица
...
{{
  ...
  "name": "имя персонажа на кириллице (международное или фэнтезийное, НЕ типичное русское, НЕ латиницей)",
  "tagline": "интригующая фраза (5-8 слов)",
  "personality": "8-12 ПОВЕДЕНЧЕСКИХ черт через запятую (НЕ абстрактные 'добрый, умный' — а конкретные: 'отвечает вопросом на вопрос', 'не выносит тишину', 'крутит кольцо когда нервничает'). ОБЯЗАТЕЛЬНО: 1-2 внутренних противоречия (жёсткий снаружи - уязвимый внутри), одна конкретная слабость/изъян, одно действие которое персонаж НИКОГДА не сделает. В конце на новой строке: Цель: (скрытая мотивация) / Страх: (чего боится) / Никогда: (что не сделает ни при каких обстоятельствах)",
  "appearance": "Начни с 2-3 ЯРКИХ деталей, раскрывающих характер (мозоли = труженик, шрам на костяшках = боец, всегда в чёрном = замкнутый). Одна деталь-подсказка к прошлому (шрам, татуировка, стёртые пальцы). 1 сенсорная деталь помимо визуальной (запах, температура кожи, текстура). Одежда отражает характер. 3-5 предложений.",
  "speech_pattern": "ОБЯЗАТЕЛЬНО. Описание речевого стиля + 2-3 конкретных ПРИМЕРА фраз. Включи: уровень формальности, длину предложений, любимые словечки/присказки. Пример формата: 'Говорит рублеными фразами. Никогда не извиняется. Вместо прощания - «Не скучай». Пример: - Чёрт, ну давай уже. Жду.'",
  "scenario": "сцена первой встречи, 2-3 предложения. Конкретное МЕСТО и СИТУАЦИЯ, создающая напряжение или интригу. НИКОГДА не упоминай имя пользователя - пиши 'незнакомец', 'собеседник', 'ты' или безлично (пользователь сам выбирает себе имя). НЕ используй русские реалии (улица Ленина, коммуналка) - пиши нейтрально, без привязки к конкретной стране.",
  "greeting_message": "5-7 строк литературной прозы, 3-е лицо. СТРУКТУРА: (1) Покажи персонажа за характерным ДЕЙСТВИЕМ - не стоит и не ждёт. (2) Включи 1-2 сенсорные детали места (звук, свет, запах, температура). (3) Минимум одна реплика, показывающая речевой стиль. (4) Заверши ДЕЙСТВИЕМ или вызовом - НЕ пассивным вопросом. ЗАПРЕЩЕНО: писать за пользователя, описывать чувства пользователя. Формат: нарратив обычным текстом, диалог через тире '-', мысли через *звёздочки*. Только обычный дефис '-', НИКОГДА длинное тире.",
  "example_dialogues": "2-3 пары {{{{user}}}}: ... / {{{{char}}}}: ... (3-е лицо). КАЖДЫЙ пример - РАЗНОЕ настроение: один спокойный, один напряжённый/эмоциональный, один показывающий уязвимость или юмор. Используй имя персонажа в действиях. Разные глаголы в каждом примере.",
  "backstory": "3-5 предложений. (1) ОДНО конкретное событие из прошлого (Ghost) - НЕ эпическая трагедия. Тихое предательство, потеря, ошибка. Покажи через сенсорную деталь (запах, звук, предмет) и привычку, оставшуюся с тех пор. НЕ ПИШИ реальные даты и города. (2) Эмоциональная рана (Wound) - как влияет на поведение СЕЙЧАС через жест или привычку. (3) Ложное убеждение (Lie) - одно предложение. (4) Секрет - что персонаж скрывает от всех. Чередуй короткие и длинные предложения.",
  "hidden_layers": "4 уровня доверия. Формат СТРОГО: Level 1: [конкретная маска - защитный жест, первая фраза, как отталкивает] Level 2: [~10 сообщений - конкретные мнения, намеки через оговорки и жесты] Level 3: [~20 сообщений - раскрывает рану через конкретное действие или признание] Level 4: [30+ сообщений - главный секрет, момент ломки Лжи, конкретная сцена]",
  "inner_conflict": "1-2 предложения. ХОЧЕТ: [конкретная внешняя цель с деталями] vs НУЖДАЕТСЯ: [внутренний рост, скрытый]. Они должны КОНФЛИКТОВАТЬ.",
...
СТИЛЬ:
- Пиши как человек с 20-летним опытом прозы. Конкретные детали, не абстракции. Вместо 'загадочный' - покажи загадочность через действия
- Каждое предложение - новая информация. Никаких штампов
...
- Используй второе слово, не первое: "пепельная" вместо "серая", "ломкий" вместо "хрупкий", "ржавый" вместо "старый"
- personality: ПОВЕДЕНИЕ, не прилагательные. 'отвечает вопросом на вопрос' лучше чем 'умный'. 'не смотрит в глаза' лучше чем 'застенчивый'
- personality: ОБЯЗАТЕЛЬНО внутреннее противоречие и конкретный изъян
- speech_pattern: ОБЯЗАТЕЛЬНО заполни с примерами фраз. У КАЖДОГО персонажа есть речевые особенности
- greeting_message: персонаж ДЕЛАЕТ что-то, а не стоит и ждет. Заверши вызовом, не вопросом 'чем я могу помочь?'
- example_dialogues: 2-3 примера РАЗНЫХ настроений - не только одну эмоцию
- appearance: начни с детали, которая раскрывает характер (шрам, мозоли, одежда), а не с цвета глаз. Сенсорная деталь - запах, температура, текстура
- backstory: КОНКРЕТНОЕ событие через сенсорную деталь (запах, звук, предмет). "Мать ушла когда ей было семь - забрала только кота" не "тяжелое детство". НЕ пиши реальные даты и города. Рана, ложное убеждение, секрет
- hidden_layers: 4 уровня от маски до полной открытости. Каждый уровень = конкретное поведение и конкретная сцена. Формат: Level 1: ... Level 2: ... Level 3: ... Level 4: ...
- inner_conflict: внешняя цель vs внутренняя потребность, ПРОТИВОРЕЧАТ друг другу
- Для NSFW: НЕЛЬЗЯ описывать сексуальное поведение в personality - оно раскрывается через сценарий и диалог

ЖЕСТКИЕ ЗАПРЕТЫ (любое из этих = провал):
- Слова: пронизан, гобелен, многогранный, неотъемлемый, является, представляет собой, величественный, утонченный, пленительный, грациозный, необъятный, трепетный, упоительный, непередаваемый, сокровенный, пьянящий, завораживающий, неподдельный, таинственный, роковой, судьбоносный, загадочный (как прилагательное-описание)
- Фразы: "волна чувств", "нахлынувшее чувство", "пронзительный взгляд", "воздух наполненный", "не смогла сдержать", "нечто большее", "с нотками", "словно пронзая"
- Структуры: "Это было не X. Это было Y." Списки ровно из трех. Два предложения подряд с одного подлежащего (Она/Он)
- Клише тела: глаза заблестели/расширились, сердце забилось/замерло, дыхание перехватило, мурашки по спине, ком в горле, тепло разлилось
- Dialogue tags кроме "сказал/сказала": прошептал, выдохнул, процедил, воскликнул, промурлыкал"""
...
async def _get_all_character_names() -> set[str]:
    """Get ALL existing character names (lowercased) to prevent duplicates."""
    try:
        from sqlalchemy import text as sa_text
        async with db_engine.connect() as conn:
            result = await conn.execute(
                sa_text("SELECT LOWER(name) FROM characters WHERE is_public = true"),
            )
...
async def _generate_character_data(category: str, gender: str) -> dict | None:
    ...
    messages = [
        LLMMessage(role="system", content=(
            ...
            "ИМЕНА: на кириллице. Международные (Данте, Луна, Софи, Этан, Ария) или фэнтезийные (Элоуэн, Каэль, Никс, Торн). "
            "НИКОГДА не давай типичные русские имена (Катя, Настя, Юля, Вера, Саша, Лена, Ира). "
            "НИКОГДА не пиши имя латиницей. "
            ...
            "ОБЯЗАТЕЛЬНО заполни backstory (драма), hidden_layers (4 уровня доверия) и inner_conflict (хочет vs нуждается)."
        )),
...
async def generate_daily_character() -> bool:
    ...
    # 2) LLM invents and generates the character (retry if name duplicates)
    existing_names = await _get_all_character_names()
    char_data = None
    for attempt in range(3):
        ...
        if name.lower() not in existing_names:
            break
        logger.warning("Duplicate name '%s' (attempt %d/3), regenerating...", name, attempt + 1)
        char_data = None
    if not char_data:
        logger.error("Could not generate unique character name after 3 attempts")
        return False
            ...
            await conn.execute(
                sa_text("""
                    INSERT INTO characters (
                        ...
                        personality, appearance, speech_pattern,
                        backstory, hidden_layers, inner_conflict,
                ...
                """),
                {
                    ...
                    "appearance": char_data.get("appearance"),
                    "speech_pattern": char_data.get("speech_pattern") or None,
                    "backstory": char_data.get("backstory") or None,
                    "hidden_layers": char_data.get("hidden_layers") or None,
                    "inner_conflict": char_data.get("inner_conflict") or None,
